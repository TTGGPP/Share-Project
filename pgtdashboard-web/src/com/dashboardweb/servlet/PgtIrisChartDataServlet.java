...
import com.dashboardweb.vo.PgtIrisVO;
import com.google.gson.Gson;
import com.google.gson.stream.JsonReader;

@WebServlet(urlPatterns = { "/pgt/iris-chart-data" })
public class PgtIrisChartDataServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String path = 
				"http://127.0.0.1:5000/pgt/iris-list?species=";
		...
		if (status == 200) {
			...
			PgtIrisVO[] irisList = gson.fromJson(reader, PgtIrisVO[].class);
			
			double t1 = 0, t2 = 0, t3 = 0, t4 = 0;
			for (int i = 0; i < irisList.length; i++) {
				PgtIrisVO iris = irisList[i];
			...
			areaChartData.append( new Double[] { t1/irisList.length,
												 t2/irisList.length,
												 t3/irisList.length,
												 t4/irisList.length,} );
